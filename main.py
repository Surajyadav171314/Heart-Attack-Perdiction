...
def analysis():
    ...
    try:
        ...
        messagebox.showerror("Mission", "Please select fbs!")
        return

    try:
        I=selection3()
    except:
        messagebox.showerror("Mission", "Please select Exang!")
        return

    try:
        C=int(selection4())
    except:
        messagebox.showerror("Mission", "Please select cp!")
        return
...
#Lets check all are working or not
    ...
    canvas.get_width_widget().pack(side=tk.BOTTOM,fill=tk.BOTH,expand=True)
    canvas._tkcanvas.place(width=250,height=250,x=600,y=240)

    ##second graph
    f2 = Figure(figsize=(5,5),dpi=100)
    a2 = f2.add_subplot(111)
    a2.plot(["Sex","trestbps","chol","thalach"],[A,D,E,H])
    canvas2 = FigureCanvasTkAgg(f2)
    canvas2.get_width_widget().pack(side=tk.BOTTOM,fill=tk.BOTH,expand=True)
    ...
    canvas3.get_width_widget().pack(side=tk.BOTTOM,fill=tk.BOTH,expand=True)
    ...
    canvas4.get_width_widget().pack(side=tk.BOTTOM,fill=tk.BOTH,expand=True)
...
def Info():
    ...
    Icon_window.geometry("700*600+100+100")
    ...
    Label(Icon_window,text="fbs - fasting blood sugar > 120 mg/ mg = true; 0 = false)",font="arial 11").place(x=20,y=250)
    Label(Icon_window,text="restecg - resting electrocardiographic results (0 = normal; 1 = having ST-T; 2 = hypertr",font="arial 11").place(x=20,y=280)
...
#header section 2
logo=PhotoImage(file="Images/header.jpg")
...
Label(Heading_entry,text="Birth Year",font="arial 13",bg="#df2d4b",fg=framefg).place(x=430,y=0)


Entry_image=PhotoImage(file="Images/Rounded Rectangle 1.png")
Entry_image2=PhotoImage(file="Iamges/Rounded Rectangle 2.png")
...
##radio Button ###
Label(Detail_entry,text="sex:",font="arial 13",bg=framefg,fg=framebg).place(x=10,y=10)
Label(Detail_entry,text="fbs:",font="arial 13",bg=framefg,fg=framefg).place(x=180,y=10)
Label(Detail_entry,text="exang:",font="arial 13",bg=framefg,fg=framebg).place(x=335,y=10)
...
cp_combobox=Combobox(Detail_entry,values=["0=typical angina","1=atypical angina","2= non-anginal pain","3=asymptomatic"],font="arila 12",state="r",width=14)
restecg_combobox=Combobox(Detail_entry,values=["0","1","2"],font="ar ila 12",state="r",width=11)
slope_combobox=Combobox(Detail_entry,values=["0= upsloping","1= flat","2= downsloping"],font="arila 12",state="r",width=12)
ca_combobox=Combobox(Detail_entry,values=["0","1","2","3","4"],font= "arila 12",state="r",width=14)
thal_combobox=Combobox(Detail_entry,values=["0","1","2","3"],font="a rila 12",state="r",width=14)
